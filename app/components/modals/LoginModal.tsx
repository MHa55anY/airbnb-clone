...
const LoginModal: FC<ModalProps> = ({
    ...
    return (
        <>
            <div 
            className="justify-center items-center flex overflow-x-hidden overflow-y-hidden fixed inset-0 z-50 outline-none bg-neutral-800/70">
                <div 
                className="relative w-full md:w-4/6 lg:w-3/6 xl:w-2/5 my-6 h-full lg:h-auto md:h-auto">
                    ...
                    className={`translate duration-300 h-full ${showModal ? 'translate-y-0' : '-translate-y-full'} ${showModal ? 'opacity-100' : 'opacity-0'}`}>
                        <div 
                            className="traslate h-full lg:h-auto md:h-auto border-0 rounded-lg shadow-lg realtive flex flex-col w-full bg-white outline-none focus:outline-none"
                        >
                            {/* HEADER */}
                            <div 
